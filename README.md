# Hi 👋, I'm [Your Name]

🎓 **AI & Data Science Undergraduate**  
💻 **Web Developer | Open Source Contributor**  
🤖 **Machine Learning Enthusiast**  
🌱 Passionate about building impactful tech and contributing to open source communities

---

## 🚀 About Me

- 🎓 AI & Data Science student with strong academic performance  
- 💻 Web developer experienced in building modern, scalable applications  
- 🧠 Actively learning **Machine Learning & AI concepts**  
- 🌍 Enthusiastic open-source contributor (aiming for **GSoC 2026**)  
- 🏫 Former **GeeksforGeeks Campus Ambassador**  
- 🏆 Recipient of the **Precious Saint Chavara Exemplar Award** for:
  - Academic excellence  
  - Dedicated service in education  
- 🎤 Strong **public speaker** with proven **leadership qualities**

---

## 🛠️ Tech Stack

**Languages:**  
- C  
- Python  
- Java  

**Web Development:**  
- HTML, CSS, JavaScript  
- Backend & APIs (learning & building)

**Machine Learning:**  
- Python for ML  
- Currently exploring core ML algorithms & applications

**Tools & Platforms:**  
- Git & GitHub  
- Linux  
- VS Code  

---

## 🌱 What I'm Currently Doing

- 📌 Contributing to open source (Apache / OWASP / PSF focused)  
- 📖 Deepening my knowledge in Machine Learning & AI  
- 🔧 Building real-world projects to strengthen fundamentals  
- 🤝 Actively engaging with open-source communities  

---

## 🎯 Goals

- ✅ Become a consistent open-source contributor  
- 🚀 Get selected for **Google Summer of Code (GSoC) 2026**  
- 🧠 Grow as an AI & ML engineer  
- 🌍 Build technology that positively impacts society  

---

## 🤝 Let's Connect

- 💼 LinkedIn: *(add link)*  
- 🌐 Portfolio: *(add if available)*  
- 📫 Email: *(optional)*  

---

⭐ *Feel free to explore my repositories and connect — I’m always open to collaboration and learning!*
